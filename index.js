#!/usr/bin/env node
// NMLP MCP server — local stdio transport.
//
// A standalone Model Context Protocol server you can run locally (Claude
// Desktop, Cursor, Continue.dev, or `npx nmlp-mcp`) or in a container. It
// implements twelve tools for antiquarian first-edition identification and the
// Albuquerque book-donation API. tools/list is served entirely from local code;
// the reference-data tools fetch the New Mexico Literacy Project's public,
// CC BY 4.0 JSON API at call time (single source of truth), while
// nmlp_decode_number_line runs fully offline.
//
// Spec: https://modelcontextprotocol.io/specification/
// Hosted twin (HTTP transport): https://newmexicoliteracyproject.org/api/mcp
//
// Public, no auth.
...
const server = new Server(
  ...
  {
    capabilities: { tools: {} },
    instructions: "First-edition identification (points of issue, number-line decoding, publisher rules) for 6,700+ verified titles / 850+ publishers (CC BY 4.0, DOI " + DATASET_DOI + "), plus the Albuquerque book-donation API. Call nmlp_check_coverage before nmlp_schedule_pickup. nmlp_schedule_pickup contacts a real human operator — only submit with explicit user consent."
  }
);
...
async function main() {
  ...
  console.error(`nmlp-mcp v${VERSION} — stdio transport ready (${TOOLS.length} tools).`);
}

main().catch((err) => {
  console.error("nmlp-mcp fatal:", err);
  process.exit(1);
});
